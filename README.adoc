...
| user     |          |                 | The database username to be used for database creation. If multiple users are required then enter a comma-separated list of usernames (no space). Limit usernames to 118 chars maximum.  Accepted chars are: upper case letters, lower case letters, digits, and underscore.
